...
//Update the stepper. This should be called once per millisecond
void Stepper_Update(Stepper* stepper)
{
	if(stepper -> stepsLeft > 0)
	{
		if(stepper -> stepTicks >= STEP_TICKS)
		{
			stepper->stepTicks = 0;
			if(stepper->stepPinValue==0)
			{
				stepper->stepPinValue=1;
