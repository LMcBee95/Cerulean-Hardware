...
#define STEP_TICKS 2 //THe number of milliseconds to wait between toggling the step pin.
...
struct Stepper_st{
	...
	int stepsLeft; //The number of steps left to take
	int angle;
	uint8_t stepPinValue; //The current value of the stepPin
	uint8_t stepTicks;    //The number of ticks since the last time the stepper was updated.
...
//Update the stepper.  This function should be called once every millisecond.
