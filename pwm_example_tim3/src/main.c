...
void anologWrite(uint16_t TIMx, uint16_t CCRx, uint16_t period, uint8_t dutyCycle)
{
	TIMx->CCRx = (period + 1) * dutyCycle / 255.0;
}


int main(void)
{
	period = uint16_t period = initialize_timers(525000, 1);
	
	while(1)
	{
			TIM3->CCR1 = (period + 1) * 50.0 / 255.0; //Sets the dury cycle of the pulse width modulation. The duty cycle is how long the pin stay on during a cycle.
